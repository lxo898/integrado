...
class Reservation(models.Model):
    ...
    @property
    def resources_desc(self):
        """Descripción texto de recursos usados."""
        items = []
        for rr in self.resources_used.all():
            items.append(f"{rr.resource.name} ({rr.quantity})")
        return ", ".join(items)
...
class ReservationResource(models.Model):
    """Recursos solicitados en una reserva (con cantidad)."""
    reservation = models.ForeignKey(Reservation, on_delete=models.CASCADE, related_name="resources_used")
    resource = models.ForeignKey(Resource, on_delete=models.PROTECT, related_name="usage")
    quantity = models.PositiveIntegerField(default=1)
...
class Profile(models.Model):
    ...
    # Asociaciones
    area = models.ForeignKey(Area, on_delete=models.SET_NULL, null=True, blank=True, related_name="profiles")
    carrera = models.ForeignKey(Carrera, on_delete=models.SET_NULL, null=True, blank=True, related_name="profiles")
